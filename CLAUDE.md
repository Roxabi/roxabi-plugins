...
- **Shared-source TS files** — two mechanisms; choice depends on whether the file ships at runtime:
  - **Copy-sync** (runtime files) — dev-core canonical byte-copied into dev-init by `bun run sync:shared`; copies carry `// @generated` header, byte-equality-gated by `tools/validate_plugins.py --check shared-sources-sync` (CI + lefthook). Manifest: `tools/shared-sources.json`. ¬hand-edit copies — edit dev-core canonical only. Currently: `config-helpers.ts` + `__tests__/config.test.ts`.
  - **Shared-import** (test-only files) — single file under `plugins/shared/__tests__/` imported directly via relative path by both plugins' test files. No copy, no `@generated` header, no sync gate needed. Currently: `detect-github-repo.suite.ts`.
  - **Selection rule:** runtime source (ships with installed plugin) → copy-sync (cross-plugin runtime imports forbidden by marketplace self-containment). Test-only source (never ships) → shared-import preferred (no duplication, no gate).

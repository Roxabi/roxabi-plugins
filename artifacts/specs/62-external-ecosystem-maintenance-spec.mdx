...
**Directory convention (added post-analysis):** `plugins/` = native Roxabi plugins. `external/` = curated/vendored external plugins. Both listed in `marketplace.json`.
...
3. Copies plugin into `external/<name>/`, rewrites frontmatter if needed, adds to `marketplace.json` with `"source": "./external/<name>"`
...
Same as above but step 3 uses `git subtree add --prefix=external/<name> <url> <branch> --squash`. `subtree_prefix` field in registry (`external/<name>`) enables future `git subtree pull`.

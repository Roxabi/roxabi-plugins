...
classDiagram
  ...
  table_formatter ..> topo_sort : topoSort('string-asc')
...
| E9 | D17 | graph.ts:33 (input-order) vs table-formatter.ts:245 (string-asc) | topo-sort.ts (new) `topoSort(ids,getUpstream,tieBreak)` | graph→`'input-order'`; table-formatter→`'string-asc'` | **order-equivalence tests required**; renamed from numeric-asc during impl — bare .sort() is lexicographic, not numeric |
...
- [ ] D17: a single `topoSort` is used by both `graph.ts` and `table-formatter.ts`, and new unit tests assert output ordering against **explicit expected-order cases** (linear / tie / cycle) that reproduce each caller's current ordering for both `input-order` and `string-asc` modes.

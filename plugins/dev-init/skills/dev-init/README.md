...
Setting up a project for dev-core involves three distinct concerns (stack config, CI pipelines, commit standards). `/dev-init` runs them in the correct order, each idempotently, so you can safely re-run after partial failures or configuration changes.
...
/dev-init --skip-axial   Skip axial ADR interview
```

**Not** the same as the host built-in `/init` (CLAUDE.md scaffold). Use `/dev-init`.

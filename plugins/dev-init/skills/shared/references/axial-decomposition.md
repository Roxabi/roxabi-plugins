...
This asymmetry is intentional. A spec may add `infrastructure/` changes without proposing a new adapter (e.g., refactoring existing stage wiring). In that case, the axial-adr concern is not relevant at the spec level because no new axis-crossing is being proposed — but it becomes relevant at dev-review if the diff shows structural drift. The two gates are complementary: `/spec` catches intent-level N×M violations, `/dev-review` catches implementation-level ones.

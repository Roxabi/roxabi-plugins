...
/**
 * Shared test suite for `ghGraphQL` and `run` from github-adapter.
 *
 * **Caller contract — required module-scope setup.** The caller must:
 *
 * ```ts
 ...
 * ```
 */
export function registerGithubSuite(opts: {
  ghGraphQL: (query: string, variables: Record<string, unknown>) => Promise<unknown>
  run: (cmd: string[]) => Promise<string>
  mockFetch: ReturnType<typeof vi.fn>
}) {
  const { ghGraphQL, run, mockFetch } = opts

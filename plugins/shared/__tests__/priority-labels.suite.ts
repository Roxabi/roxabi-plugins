...
/**
 * Shared test suite for PRIORITY_LABEL_MAP, PRIORITY_LABELS_SET, and syncPriorityLabel.
 *
 * **Caller contract — required hoisted mock.** The caller must hoist at module scope:
 *
 * ```ts
 * vi.mock('../adapters/github-adapter', () => ({ updateLabels: vi.fn() }))
 * ```
 *
 * Without this mock the `updateLabels` spy won't be set up before the module loads.
 */
export function registerPriorityLabelsSuite(opts: {
  updateLabels: ReturnType<typeof vi.fn>
  PRIORITY_LABEL_MAP: Record<string, string>
  PRIORITY_LABELS_SET: Set<string>
  syncPriorityLabel: (issueNum: number, priority: string) => Promise<void>
}) {
  const { updateLabels: mockUpdateLabels, PRIORITY_LABEL_MAP, PRIORITY_LABELS_SET, syncPriorityLabel } = opts

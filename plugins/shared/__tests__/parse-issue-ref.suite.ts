...
export function registerParseIssueRefSuite(opts: {
  parseIssueRef: (input: string) => IssueRef | undefined
  parseIssueRefs: (input: string) => IssueRef[]
}) {
  const { parseIssueRef, parseIssueRefs } = opts

...
interface WorkspaceProjectLike {
  ...
  projectId: string
  label: string
  fieldIds?: ProjectFieldIdsLike
}

export function registerResolveFieldIdsSuite(opts: {
  resolveFieldIds: (project: WorkspaceProjectLike) => ProjectFieldIdsLike
  fieldIdForSlot: (project: WorkspaceProjectLike, slot: 'col2' | 'col3') => string | undefined
  STATUS_FIELD_ID: string
  SIZE_FIELD_ID: string
  PRIORITY_FIELD_ID: string
  STATUS_OPTIONS: unknown
  SIZE_OPTIONS: unknown
  PRIORITY_OPTIONS: unknown
}) {
  const {
    resolveFieldIds,
    fieldIdForSlot,
  ...
  describe('resolveFieldIds', () => {
    ...
    it('throws when fieldIds present but status missing', () => {
      const project = {
        ...
        projectId: 'p',
        label: 'test-proj',
        // tests the JSON-deserialization path: workspace.json written by hand
        // or a future version may contain fieldIds without status
        fieldIds: { col2: 'C1', col3: 'C2' },
      }

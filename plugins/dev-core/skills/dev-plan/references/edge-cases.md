...
| Scenario | Behavior |
|----------|----------|
| No spec found | Suggest `/R-spec` or `/R-dev`. Stop. |
...
| Multi-slice spec | Step 2e: auto next unimplemented when unique; else default first parallel root + χ in summary. 1 slice/run default. Re-run `/R-dev-plan` for rest. |
| Mid-plan double approve | Forbidden — sole gate is Step 6 Executive Summary + free-form (2f removed) |
| All slices implemented | Detect via code/tests. Suggest `/R-dev-review`. |

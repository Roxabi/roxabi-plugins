...
description: Implementation plan — tasks, agents, file groups, dependencies. Triggers: "dev-plan" | "plan" | "plan this" | "implementation plan" | "break it down" | "plan this feature" | "how should we build this" | "make a plan" | "create a plan" | "break this down into tasks" | "task breakdown".
version: 0.6.1
...
<!-- Generated by /dev-plan. Used by /implement to resume tasks on session restart. -->
- T1: {task_id} — {subject}
- T2: {task_id} — {subject}
...
```
...
`/dev-plan` runs only for τ ∈ {F-lite, F-full} (τ=S skips plan) → approval always lands on a **compact pause** before `/implement`.

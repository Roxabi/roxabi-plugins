...
**Subdomain split (multi-chunk):** one allocate call (`--diff-list` full Δ + `--chunk-list` per chunk). `--chunks` derived from `--chunk-list` (omit `--chunks`; disagreement → warning, file count wins). Empty `--chunk-list` file → exit 1. Scope check both directions: chunk path ∉ `--diff-list` ∨ in two chunks → warning; Δ path ∈ no chunk → **one aggregated warning** (that file is reviewed by nobody; ¬exit 1 — the chunker drops binaries that `--name-only` lists). Spawn exactly `chunk_agents[i]` on `c_i.files`. `Spawned roster (this review): {agents}` = union. COLLAPSE_ONCE once per review. `max_agents` per-chunk; `max_agents_review` default 0 = off (floors ¬capped). R-recall is Phase 3b (not per-chunk Lane A). R-finding-verifier is Phase 4 (once per review).
...
3. `## Code Review` header, then **block order — top to bottom**. Both axis blocks are **review output**, ¬PR-comment decoration: emit them unconditionally (this phase copies; ¬vanish when ¬∃ PR — same rule as Phase 4 step 7). ¬∃ PR → render the whole block order to the user and skip only the `gh pr comment` call.
   ...
   **`/R-fix` partition (load-bearing):** `fix/SKILL.md` Phase 1 parses Conventional Comments from the **whole** PR comment body and only strips the finding-verifier `<details>` (step 1a). There is no file:line dedup. Therefore:
   - `## Spec` ∧ `## Standards` rows MUST be non-CC-shaped. The rule is a **shape over the CC stems, ¬a denylist of exact strings**: no line in either block may match `^\s*[-*]?\s*(issue|suggestion|todo|nitpick|thought|question|praise)(\([a-z-]+\))?:` — the optional decoration is what catches `issue(security):` and `nitpick(style):`, which a literal list misses, and `/R-fix` keys off the stem (`actionable := {issue, suggestion, todo, nitpick}`, `fix/SKILL.md`). Anchor on those stems, ¬on a bare `[a-z]+:` — that would also ban this block's own required `smells: RAS`. § Finding categories (immediately before Phase 4) enumerates the stems in use; a stem added there extends this rule by construction. Point at the findings pile in prose (`see findings`) — never embed a live label.

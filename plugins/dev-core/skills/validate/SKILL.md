...
**falsify** (optional, `--full` only): run if `{commands.test:falsify}` is defined **or** `package.json` has script `test:falsify`. Else σ := ⏭ skip. ¬fail the suite because the consumer repo has no falsify script.

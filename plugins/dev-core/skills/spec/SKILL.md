...
`skill: "interview", args: "--promote artifacts/analyses/{N}-{slug}.mdx"` (or frame path if ¬analysis).
...
On approval → commit: `git add artifacts/specs/{N}-{slug}.mdx` + commit per CLAUDE.md Rule 5.

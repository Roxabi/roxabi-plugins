 ...
 *  2. Check milestones M0/M1/M2 — warn if missing, no mutation
 *  3. Verify at least one item is in the hub project (lightweight probe)
 */
...
export async function enrollRepo(opts: {
  ...
  const { org, repo, projectId, dryRun = false } = opts
  ...
  // Step 3: Check milestones M0/M1/M2 — warn only, no mutation

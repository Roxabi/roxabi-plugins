...
| `tools/licenseChecker.ts` missing | Run `/R-ci-setup` — the copy lives in `ci-setup/cookbooks/hooks.md` step g |
| trufflehog not in lefthook | Run `/R-ci-setup` — regenerates `lefthook.yml` |
| principal freeze not in lefthook | Run `/R-ci-setup` Phase 2e or `bun init.ts seed-principal-freeze` |
| license check not in lefthook | Run `/R-ci-setup` — regenerates `lefthook.yml` |
...
| no hook manager at all | Run `/R-ci-setup` (lefthook) or add `.pre-commit-config.yaml` |

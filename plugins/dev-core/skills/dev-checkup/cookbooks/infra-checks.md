...
```bash
bun -e "
import { getWorkspacePath, readWorkspace } from '../../shared/adapters/workspace-helpers.ts'
...
2. **App token** — missing `ROXABI_CI_APP_PRIVATE_KEY` / `ROXABI_CI_APP_ID` → provision per `../../ci-setup/cookbooks/github-app.md`. D✅("roxabi-ci App credentials").

...
interface CheckPrereqsResult {
  ...
  gitRemote: { ok: boolean; url: string; owner: string; repo: string }
}

export function registerPrereqsSuite(opts: { checkPrereqs: () => CheckPrereqsResult }) {
  const { checkPrereqs } = opts

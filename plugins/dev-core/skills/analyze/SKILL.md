...
| options / alternatives / cheaper route | Fresh host-native read-only exploration worker with empty context — each host maps the role (Claude→`Explore`, OMP→`scout`, etc.; no durable options manifest). Claude example: `Task(subagent_type: "Explore")`. Prompt: `Read-only morphological option-space sweep. Follow ${CLAUDE_SKILL_DIR}/references/options-lattice.md exactly. Isolation: no prior context. Subject: <α path>. Never write files. Return the lattice + verdict.` → fold survivors/verdict into α if user asks → **re-print Executive Summary → STOP again**. |

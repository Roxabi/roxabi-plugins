...
Build the map from the SC→Test matrix: each unit/FI row → `{ sc_id, sources: [<priced source paths>], test_cmd: "{commands.test_file} {test_file}" }` (`commands.test_file` when `.dev/stack.yml` sets it, else `{commands.test}`). That shape is **enforced**, not advisory: the helper re-derives the command from the contract + plain relative test file paths the snapshot carries and runs it as argv, with no shell. If `commands.test` cannot run one named file (a turbo root script, `unittest discover`), every row ends `restore-failed` — set `commands.test_file` (e.g. `bunx vitest run`). Test paths reach argv, so they are an allowlist (`[A-Za-z0-9_./@+-]`, no leading `.`, no `..`): a route-group path like `app/(auth)/x.test.tsx` cannot be a falsify test path. Sources are only hashed and deleted inside the snapshot: any relative path with no empty, `.` or `..` segment (dot-dirs are fine). Refusals: `refused-test-cmd:row<i>` — a row that is not that shape (a pipe, a `;`, another runner, a `..` path, a test path the snapshot does not carry), a malformed row, or a bad source; the whole map is refused and every refused row is named on stderr. `missing-test-command` / `unsupported-test-command` — the contract's command is absent, or is not one single-line plain command (shell syntax, a `VAR=` prefix, a multi-line value): fix `.dev/stack.yml`, re-mapping will not help. `runner-error` — the input made the helper raise (stderr says why). ADR-019 §2c. Isolation is **copy at HEAD with the working tree overlaid** inside the helper (¬repo-global `git stash` as the public API) — which is why the `git add`-but-not-commit precondition above is enough: the snapshot carries your uncommitted work. ADR-019 §2a.

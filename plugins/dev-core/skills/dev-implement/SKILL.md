...
Build the map from the SC→Test matrix: each unit/FI row → `{ sc_id, sources: [<priced source paths>], test_cmd: "{commands.test} {test_file}" }`. That shape is **enforced**, not advisory: the helper re-derives the command from `.dev/stack.yml` `commands.test` + plain relative test file paths and runs it as argv, with no shell. Paths are an allowlist (`[A-Za-z0-9_./@+-]`, no `..`) because `bun run` / `npm run` re-shell trailing args — a route-group path like `app/(auth)/x.test.tsx` cannot be a falsify row. Refusals: `refused-test-cmd:row<i>` — a row that is not that shape (a pipe, a `;`, another runner, a `..` path, a test path the snapshot does not carry) or an absolute/escaping source; the whole map is refused and every refused row is named on stderr. `missing-test-command` / `unsupported-test-command` — `commands.test` itself is absent, or is not one plain command (shell syntax, a `VAR=` prefix): fix `.dev/stack.yml`, re-mapping will not help. ADR-019 §2c. Isolation is **copy at HEAD with the working tree overlaid** inside the helper (¬repo-global `git stash` as the public API) — which is why the `git add`-but-not-commit precondition above is enough: the snapshot carries your uncommitted work. ADR-019 §2a.

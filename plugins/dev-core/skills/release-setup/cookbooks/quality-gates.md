...
**Critical:** `QG_FILE_EXTS` is written with double-quotes in the generated file (`QG_FILE_EXTS="js jsx ts tsx"`) because multi-word values (node repos) would otherwise cause shell word-splitting errors when qg.conf is sourced — the unquoted tokens after the first would be interpreted as commands.

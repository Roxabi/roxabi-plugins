...
Distinct from `/R-dev-review`, which already embeds the `R-adversarial` *agent* on PR diffs. This skill is the entry point when there is no PR yet — only a shape, proposal, or argument.

## Usage

```
/R-adversarial "we should pin every workflow to main"
/R-adversarial --issue 374
/R-adversarial --analysis artifacts/analyses/374-release-gate-analysis.md
/R-adversarial --spec artifacts/specs/374-release-gate-spec.md --write
```
...
3. **Attack** — spawn the `R-adversarial` agent with shape-aware lens guidance.
...
| Skill | Posture |
|-------|---------|
| `/R-adversarial` | Kill the design |
| `/R-advisory` | Strengthen and advise (constructive) |
| `/R-dev-review` | Diff/PR multi-domain review (includes `R-adversarial` agent) |
| `/R-analyze` | Structured shape exploration (no attack) |

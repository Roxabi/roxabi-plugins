...
**Who reads this:** `roxabi-issues:issue-triage` (writer) · label/type bootstrap via `/dev-init` / org tooling · `lyra/scripts/dep-graph` · `roxabi-live` worker (repo-centric GraphQL readers).

 ...
 *
 * dev-core's copy pointed both non-green signals at `/R-dev-checkup`. omp-build
 * ships no such skill (ADR-020 §7), so the advice says what to do instead of
 * naming a command that resolves nowhere.
 *
 * Example outputs:
 *   hotfix-density: 2/10 (20%) — WARN: elevated hotfix rate since the last release
 *   hotfix-density: 1/10 (10%) — OK
 *   hotfix-density: 5/10 (50%) — PAUSE recommended: >40% hotfix rate; find the defect source before promoting
 */
export function formatResult(result: HotfixDensityResult): string {
  ...
  switch (result.gauge) {
    case 'green':
      ...
      break
    case 'warn':
      signal = 'WARN: elevated hotfix rate since the last release'
      break
    case 'pause':
      signal = `PAUSE recommended: >${(THRESHOLD_WARN * 100).toFixed(0)}% hotfix rate; find the defect source before promoting`
      break
  }
